...
use fire::{FirePit, Request, Response};
...
pub struct JsServer {
	pool: PoolHandle,
	index: Arc<String>
	// tx: mpsc::Sender<(SsrRequest, oneshot::Receiver<SsrResponse>)>
}
...
impl JsServer {
	...
	pub async fn request(&self, req: &mut Request) -> Result<Response, Error> {
		...
		let headers: HashMap<_, _> = headers.iter()
			.filter_map(|(key, val)| {
		...
		let resp = self.pool.send_req(ssr_request).await
